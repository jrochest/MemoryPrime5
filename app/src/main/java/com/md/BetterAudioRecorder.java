...
public class BetterAudioRecorder
{
	/**
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener()
	{
		public void onPeriodicNotification(AudioRecord recorder)
		{
			...
			{
				Log.e(BetterAudioRecorder.class.getName(), "Error occured in updateListener, recording is aborted");
				stop();
			}
		}
	 ...
	 * 
	 */ 
	public BetterAudioRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig,
			int audioFormat)
	{
		...
		try
		{
			...
			{ // RECORDING_UNCOMPRESSED
				...
				{ // Check to make sure buffer size is not smaller than the smallest allowed one 
					...
					Log.w(BetterAudioRecorder.class.getName(), "Increasing buffer size to " + Integer.toString(bufferSize));
				}
				
				//bufferSize = AudioRecord.getMinBufferSize(sampleRate,channelConfig,audioFormat);

				
				aRecorder = new AudioRecord(audioSource, sampleRate, channelConfig, audioFormat, bufferSize*10);
				if (aRecorder.getState() != AudioRecord.STATE_INITIALIZED)
				{
					
					throw new Exception("AudioRecord initialization failed: " + aRecorder.getState());
		...
		} catch (Exception e)
		{
			...
			{
				Log.e(BetterAudioRecorder.class.getName(), e.getMessage());
			}
			else
			{
				Log.e(BetterAudioRecorder.class.getName(), "Unknown error occured while initializing recording");
			}
			state = State.ERROR;
		}
	}
	...
	public void setOutputFile(String argPath)
	{
		...
		catch (Exception e)
		{
			...
			{
				Log.e(BetterAudioRecorder.class.getName(), "Unknown error occured while setting output path");
			}
			state = State.ERROR;
		}
	}
	...
	*/
	public void prepare()
	{
		try
		{
			if (state == State.INITIALIZING)
			{
				if (rUncompressed)
				{
					...
					{
						Log.e(BetterAudioRecorder.class.getName(), "prepare() method called on uninitialized recorder");
						state = State.ERROR;
					}
				}
				else
			...
			{
				Log.e(BetterAudioRecorder.class.getName(), "prepare() method called on illegal state");
				release();
				state = State.ERROR;
			}
		}
		catch(Exception e)
		{
			...
			{
				Log.e(BetterAudioRecorder.class.getName(), "Unknown error occured in prepare()");
			}
			state = State.ERROR;
		}
	}
	...
	public void release()
	{
		...
		else
		{
			if ((state == State.READY) & (rUncompressed))
			{
				...
				{
					Log.e(BetterAudioRecorder.class.getName(), "I/O exception occured while closing output file");
				}
	...
	public void start()
	{
		...
		{
			Log.e(BetterAudioRecorder.class.getName(), "start() called on illegal state");
			state = State.ERROR;
		}
	}
	...
	public void stop()
	{
		...
		{
			Log.e(BetterAudioRecorder.class.getName(), "stop() called on illegal state");
			state = State.ERROR;
		}
	}
